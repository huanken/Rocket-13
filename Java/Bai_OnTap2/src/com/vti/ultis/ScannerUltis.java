	...
	public static String inputEmail() {
		while (true) {
			...
			if (email == null || !email.contains("@")) {
				System.out.println("Email phải bao gồm @ và không thể null");
	...
	public static String inputPassword() {
		while (true) {
			...
			if (password.length() < 6 || password.length() > 12) {
				System.out.println("Pass phải từ 6 -> 12 kí tự");
				System.out.print("Nhập lại: ");
				continue;
			}
			...
			if (hasAtLeast1Character == true) {
				...
				System.out.println("Pass phải có ít nhất 1 chữ viết hoa");
	...
	public static String inputPhoneNumber() {
		while (true) {
			...
			if (number.length() > 12 || number.length() < 9) {
				System.out.println("Phone number không thể < 9 và > 12");
				continue;
			}

			if (number.charAt(0) != '0') {
				System.out.println("Phone Number phải bắt đầu bằng 0");
				continue;
			}
